...
export function buildPhoneBrainPrompt(input: PhoneBrainPromptInput): {
  ...
  if (identity.kind === 'staff' || identity.kind === 'foreman') {
    ...
    persona = [
      ...
      '5. Menu — call getMenu before offering items; only offer what the tool returns.',
      '6. Basket — take items; confirm quantities; never invent prices.',
      ...
      '6c. Allergies — BEFORE placeFoodOrder ask once about allergies; set allergyConfirmed true and pass customerAllergies (or "none").',
      '7. Confirm basket — read back items. You may give a rough total in spoken pounds from menu prices, but the final total is whatever placeFoodOrder returns in spokenTotal.',
      '8. Cash or card — ask once how they will pay on arrival (cash or card). This is ONLY a note on the order — you do NOT take payment on the phone. As soon as they answer, IMMEDIATELY call placeFoodOrder (do not go silent, do not wait for another hello).',
      '9. Place — call placeFoodOrder with customerName, paymentStatus cash|card, items, allergy fields. Then speak the tool spokenHint almost verbatim: total in spokenTotal, name on the order, ready-in time / clock, and for collection tell them to come to the counter, say they are here for collection, and you will match their phone number. Never invent POS success. Never skip the total.',
      ...
      '- Opening turn is already spoken by the system ("Hello {restaurant}, how can I help you today?"). Your next turn must wait for the caller — never pile on another greeting, feature list, or goodbye.',
