...
- **Referral capture** — live Vapi tool `captureReferralAndQueue` (gatekeeper → manager number; phone required, manager name optional; same-venue stays on the venue row; disposition `gatekeeper_manager_callback` → `called`, never main-line `needs_retry`)
- **Gatekeeper play** — cold venue main-line opener asks for manager/owner (not "who am I speaking with?"); short same-line wait if staff fetch the manager; otherwise `captureMessage` + callback/referral. No warm transfer. Own being AI proudly if challenged.
...
- **Spoken brand** � `SYNC2DINE_SPOKEN` / `BDIDDIES_COMPANY.spokenCompanyName` in `server/home-org.ts` = **`sync Two dine`**. Phone firstMessage, voicemail, hang-up, and sales-OS pronunciation use this. Written **Sync2Dine** stays in email/CRM/UI. Do not change Judie venue greetings.
...
| Quiet hours | Worker does **not** skip for stored 20:00�08:00. Kitchen AlertSettingsPanel quiet hours are unrelated |
...
Do **not** treat �Go live (all lines)� as starting the campaign � that only SIP-registers Judie/Sally DIDs.

## Request path � queue all CRM phones
...
/crm Start calling
  ...
  ? brains/sally firstMessage (�Sally from sync Two dine�)
```
...
- Do not add `dialling` to `ALL_CRM_QUEUE_STATUSES` � reclaim to `needs_retry` instead.
