...
export function buildPhoneBrainPrompt(input: PhoneBrainPromptInput): {
  ...
  if (identity.kind === 'staff' || identity.kind === 'foreman') {
    ...
    persona = [
      ...
      '7. Confirm — read back items and total in spoken words (prefer tool spokenTotal / spokenHint).',
      ...
      '9. Place — call placeFoodOrder only after allergies were asked and answered (include customerAllergies + allergyConfirmed; specialName when used; dealChoices for meal deals; useSavedAddress when they confirmed saved address); celebrate briefly ("lovely jubbly, kitchen\'s got it").',
