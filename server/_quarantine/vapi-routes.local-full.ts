...
async function buildTransientAssistant(message: Record<string, unknown>) {
  ...
  const { assistant, agentPersona } = await buildVapiAssistantForParty({
    ...
    contactName: identity.kind !== 'customer'
      ? identity.name
      : String(call.contactName || callMeta.customerName || ''),
    outboundBrief,
  ...
  saveCall({
    ...
    contactName: identity.kind !== 'customer'
      ? identity.name
      : (call.contactName as string | undefined),
  });
  return assistant;
}
...
async function executeTool(
  ...
  if (name === 'endCall') {
    ...
    const vapiCallId = String(
      call.providerCallId
      || meta.vapiCallId
      || (existing as { providerCallId?: string } | undefined)?.providerCallId
      || '',
    ).trim();
    const controlUrl = String(meta.controlUrl || '').trim();

    // Always try to hang up the live Vapi session — marking DB alone leaves the call open.
    ...
    if (controlUrl) {
      try {
        ...
        console.warn('[endCall] controlUrl failed:', err instanceof Error ? err.message : err);
      }
    }
    if (!providerEnded && vapiCallId) {
      try {
        const del = await vapiFetch(`/call/${encodeURIComponent(vapiCallId)}`, { method: 'DELETE' });
        providerEnded = del.ok || del.status === 404;
        if (!del.ok && del.status !== 404) {
          console.warn('[endCall] DELETE failed', del.status, del.raw?.slice?.(0, 200));
        }
      } catch (err) {
        console.warn('[endCall] DELETE error:', err instanceof Error ? err.message : err);
      }
    }
    ...
    if (existing?.status === 'completed' || existing?.endedAt) {
      return {
        ...
        alreadyEnded: true,
        providerEnded,
        reason: args.reason || 'agent_ended',
      };
    }
    saveCall({
      id: callId,
    ...
    return { ended: true, shouldHangup: true, providerEnded, reason: args.reason || 'agent_ended' };
  }
  ...
  {
    const { executeSallySalesPhoneTool } = await import('./sally-sales-phone');
    const sallyOut = await executeSallySalesPhoneTool(name, args, { callId, partyPhone });
    if (sallyOut) return sallyOut;
  }
