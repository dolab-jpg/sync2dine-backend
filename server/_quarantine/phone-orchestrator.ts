/**
 * @deprecated LEGACY turn loop — not mounted by server/index.ts or phone/vapi-routes.ts.
 * Live phone uses brains/* + vapi-assistant + vapi-routes tool handlers.
 * See docs/PHONE_ARCHITECTURE.md. Keep for softphone/historical adapters until removed deliberately.
 */
...
export async function handlePhoneTurn(body: PhoneOrchestratorRequest): Promise<{
  ...
  if (staffRoute.mode === 'staff' || staffRoute.mode === 'foreman') {
    if (isFirstTurn) {
      const greeting = `Alright ${staffRoute.name ?? 'boss'}, Sync2Dine on the line. What do you need?`;
