/**
 * After Judie placeFoodOrder: push to connected POS / commerce partner when configured.
 * No-ops safely when connectors are disabled — kitchen board still has the order.
 */
...
export async function forwardJudieOrderToProviders(
  ...
  if (!config?.enabled) {
    return { order, channel: 'none', ok: true };
  }
  ...
  if (isCommerceOutboundEnabled(config)) {
    ...
    if (push.ok) {
      ...
      return { order: synced ?? current, channel: 'commerce', ok: true };
    }
    ...
    return { order: failed ?? current, channel: 'commerce', ok: false, error: err };
  }
