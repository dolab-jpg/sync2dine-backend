...
export function isPlausibleUkStreetAddress(text: string): boolean {
  ...
  const hasStreetWord =
    /\b(street|st|road|rd|avenue|ave|lane|ln|drive|dr|close|court|ct|way|place|pl|grove|gardens?|terrace|crescent|hill|park|row|square|sq|walk|mews|yard)\b/i.test(
      withoutPostcode,
    );
  return (hasHouseNumber || hasNamedHouse) && (hasStreetWord || withoutPostcode.split(/\s+/).length >= 2);
}
