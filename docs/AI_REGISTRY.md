...
| `sally` | Sally sales | phone_brain | SaaS sales closer | `brains/sally`, `phone/sally-sales-phone.ts`, `sally/` | `brains/sally/index.ts` | `sally/prompts.ts`, phone overlay; speak **sync Two dine** | `phone_sally_sales` | Vapi DID `sally`; CRM `/crm` `allCrm` + `/calls` | platform org | live | yes |
