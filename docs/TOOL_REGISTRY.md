...
**Sally sales (live Vapi):** `getOfferTerms`, `bookIntegrationMeeting`, `bookDemo` (alias), `sendSalesFollowUp`, `recallAccountMemory`, `setCallObjective`, `researchRestaurantProfile`, `scheduleVenueCallback`, `updateVenueProfile`, `rememberPerson`, `captureReferralAndQueue` (phone required; manager name optional), plus picked: `bookCallback`, `captureLead` (venue `name` required; `contactName` optional), `captureMessage`, `classifyCallIntent`, `scheduleAppointment`, plus `endCall`. Staff mode merges staff CRM + receptionist. `setCallLanguage` is stripped on Sally phone sessions (English-only).
