/**
 * Generate Supabase TypeScript types and sync to frontend repo.
 * Requires: supabase start (local) OR linked cloud project.
 *
...
const SHARED = join(ROOT, 'shared', 'database.types.ts');
const FRONTEND_TYPES = join(
  ROOT,
  '..',
  'Bathroom Sales Estimation Platform',
  'src',
  'lib',
  'supabase',
  'types.ts',
...
function generateTypes(): string {
  try {
    ...
    console.warn('Local Supabase not running — using stub types. Run `supabase start` for full types.');
    return STUB_TYPES;
  }
}

const STUB_TYPES = `export type Json =
  | string
  | number
  | boolean
  | null
...
writeFileSync(SHARED, types);
console.log(`Wrote ${SHARED}`);

if (existsSync(dirname(FRONTEND_TYPES))) {
  mkdirSync(dirname(FRONTEND_TYPES), { recursive: true });
  writeFileSync(FRONTEND_TYPES, types);
  console.log(`Synced to ${FRONTEND_TYPES}`);
} else {
  console.warn(`Frontend path not found: ${FRONTEND_TYPES}`);
}
