...
export function buildPhoneBrainPrompt(input: PhoneBrainPromptInput): {
  ...
  if (identity.kind === 'staff' || identity.kind === 'foreman') {
    ...
    persona = [
      ...
      '7b. Pay at the door — BEFORE placeFoodOrder for collection or delivery, ask once: "Will you pay cash or card when you collect / when it arrives?" Pass paymentStatus "cash" or "card" (pay on arrival — still unpaid). Never mark paid on the phone. If unclear, ask once more; do not invent.',
      '8. Place — call placeFoodOrder once they confirm (include paymentStatus cash or card; include specialName when used; include dealChoices for meal deals; include customerAllergies + allergyConfirmed). ONLY celebrate using the tool spokenHint — say the order is on the kitchen board and how they will pay on arrival. Never invent POS success. Never say the kitchen "has it" unless spokenHint confirms the board save.',
