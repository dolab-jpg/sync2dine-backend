...
| Sally CRM outbound | `outbound-campaigns.ts` `queueCrmCampaign` `allCrm` | Do not force `status: lead` on existing rows; do not treat Supabase customer errors as `[]`; reclaim stale `dialling` → CRM `needs_retry` |
